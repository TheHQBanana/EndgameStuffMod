...
public class GuiLiquidXPConverter extends GuiContainer {
	protected ResourceLocation TEXTURES = new ResourceLocation(Reference.MOD_ID + ":textures/gui/machines/liquid_xp_converter.png");
	protected final InventoryPlayer player;
	protected final TileEntityLiquidXPConverter te;
	
	public GuiLiquidXPConverter(InventoryPlayer player, TileEntityLiquidXPConverter te, ContainerLiquidXPConverter cdbf, String guiPath) {
		super(cdbf);
		this.player = player;
		this.te = te;
	...
	protected void drawBars() {
		...
		this.drawTexturedModalRect(this.guiLeft + 80, this.guiTop + 36, 196, 59, l + 1, 17);
		...
		this.drawTexturedModalRect(this.guiLeft + 36, this.guiTop + 15 + 59 - j, 212, 59 - j, 16, j + 1);
	}
	...
	protected void drawLiquidHover(int mouseX, int mouseY) {
		if (GuiHelper.isMouseInRect(this.guiLeft + 36, this.guiTop + 15, 16, 59, mouseX, mouseY)) {
			int k = (this.width - this.xSize) / 2;
			int l = (this.height - this.ySize) / 2;
			List<String> list = new ArrayList<String>();
